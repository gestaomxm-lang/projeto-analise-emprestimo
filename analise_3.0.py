...
if processar and uploaded_files:
    with st.spinner("Processando arquivos..."):
        ...
        def mapear_colunas(df):
            ...
            for col in df.columns:
                ...
                # Hora
                    ...
                    if 'hora' not in mapeamento.values():
                        mapeamento[col] = 'hora'
            return mapeamento
        ...
        df_saida['data'] = analise_core._parse_date_column(df_saida['data'])
        df_entrada['data'] = analise_core._parse_date_column(df_entrada['data'])
        ...
        def _combinar_data_hora(df):
            if 'hora' in df.columns and 'data' in df.columns:
                ...
                def parse_hora(x):
                    if pd.isna(x) or x == '': return pd.Timedelta(0)
                    str_x = str(x).strip()
                    try:
                        # Tenta formatos comuns
                        return pd.to_timedelta(str_x)
                    except:
                        try:
                            # Tenta converter string "HH:MM:SS"
                            return pd.to_timedelta(str_x + ':00') if len(str_x) <= 5 else pd.Timedelta(0)
                        except:
                            # Se for objeto datetime.time (comum no Excel)
                            if hasattr(x, 'hour'):
                                return pd.Timedelta(hours=x.hour, minutes=x.minute, seconds=x.second)
                            return pd.Timedelta(0)
                
                # Se a coluna hora já for datetime/timedelta, ajusta
                if not pd.api.types.is_timedelta64_dtype(df['hora']):
                     # Se for datetime.time (do pandas read_excel), converte
                ...
                # Localiza para America/Sao_Paulo
                try:
                    df['data'] = df['data'].dt.tz_localize('America/Sao_Paulo', ambiguous='NaT', nonexistent='shift_forward')
                except Exception:
                    # Se falhar (ex: já tiver timezone), tenta converter
                    df['data'] = df['data'].dt.tz_convert('America/Sao_Paulo')
                    
            return df

        df_saida = _combinar_data_hora(df_saida)
        df_entrada = _combinar_data_hora(df_entrada)
        
        # --- Normalização de Nomes de Hospitais ("De-Para") ---
        def _normalizar_hospital(nome):
            if pd.isna(nome): return nome
            nome = str(nome).strip().upper()
            
            # Mapeamento explícito (De -> Para)
            de_para = {
                ...
                "HOSPITAL CASA HOSPITAL ECOR - REDE CASA": "HOSPITAL ECOR", # Exemplo hipotético se necessário
            }
            
            if nome in de_para:
                return de_para[nome]
                
            # Regra genérica: remove sufixos comuns se não estiver no de-para
        ...
        # Aplica normalização nas colunas de unidade
        if 'unidade_origem' in df_saida.columns:
            df_saida['unidade_origem'] = df_saida['unidade_origem'].apply(_normalizar_hospital)
        if 'unidade_destino' in df_saida.columns:
            df_saida['unidade_destino'] = df_saida['unidade_destino'].apply(_normalizar_hospital)
            
        if 'unidade_origem' in df_entrada.columns:
            df_entrada['unidade_origem'] = df_entrada['unidade_origem'].apply(_normalizar_hospital)
        if 'unidade_destino' in df_entrada.columns:
            df_entrada['unidade_destino'] = df_entrada['unidade_destino'].apply(_normalizar_hospital)
        
        # --- Filtragem de OFTALMOCASA ---
        # Remove registros onde a origem ou destino contém OFTALMOCASA
        termo_exclusao = "OFTALMOCASA"
        
        if 'unidade_origem' in df_saida.columns and 'unidade_destino' in df_saida.columns:
            mask_saida = df_saida['unidade_origem'].str.contains(termo_exclusao, na=False) | df_saida['unidade_destino'].str.contains(termo_exclusao, na=False)
            qtd_removida_saida = mask_saida.sum()
            if qtd_removida_saida > 0:
                df_saida = df_saida[~mask_saida]
                
        if 'unidade_origem' in df_entrada.columns and 'unidade_destino' in df_entrada.columns:
            mask_entrada = df_entrada['unidade_origem'].str.contains(termo_exclusao, na=False) | df_entrada['unidade_destino'].str.contains(termo_exclusao, na=False)
            qtd_removida_entrada = mask_entrada.sum()
            if qtd_removida_entrada > 0:
                df_entrada = df_entrada[~mask_entrada]
        # -------------------------------------------------------
        
        # Normaliza valores numéricos (remove separadores de milhar)
        if 'qt_entrada' in df_saida.columns:
            df_saida['qt_entrada'] = df_saida['qt_entrada'].apply(analise_core.normalizar_valor_numerico)
        if 'qt_entrada' in df_entrada.columns:
            df_entrada['qt_entrada'] = df_entrada['qt_entrada'].apply(analise_core.normalizar_valor_numerico)
        if 'valor_total' in df_saida.columns:
            df_saida['valor_total'] = df_saida['valor_total'].apply(analise_core.normalizar_valor_numerico)
        if 'valor_total' in df_entrada.columns:
            df_entrada['valor_total'] = df_entrada['valor_total'].apply(analise_core.normalizar_valor_numerico)
        ...
        # Processa (Usando a biblioteca core)
        progress_bar = st.progress(0, text="Iniciando...")
        
        # Wrapper para adaptar o progress_bar do Streamlit ao callback do core
        def update_progress(p, text):
            progress_bar.progress(p, text=text)

        df_res, stats = analise_core.analisar_itens(df_saida, df_entrada, limiar, update_progress)
        progress_bar.empty()
